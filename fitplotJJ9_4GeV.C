...
//setting the enviroment
  ...
  float momentum=15.0;
  Int_t nbins=18;
  Float_t low= 9.0, high= 9.7;
  // Float_t low= 0., high= 200.;
  ...
  RooRealVar VtxProb("VtxProb","VtxProb",0.05,1.);
  ...
  RooRealVar Psi1To2_dY("Psi1To2_dY","Psi1To2_dY",0.,1.5);
  ...
  RooRealVar FourMu_pT("pt","pt",momentum,200.);
  ...
  //  data = RooDataSet::read("All_OptimizedDataWithJpsiPtand4MuVtxProbCut.dat",args);
  ...
  //fixed variable
  RooRealVar mean("mean","{4#mu}_{mass}",9.39);
  //  RooRealVar mean("mean","{4#mu}_{mass}",i/10.,9.2,9.5);

  RooRealVar width("width","Width",0.01,0.005,0.05);
  //RooRealVar sigma("sigma","sigma 4mu mass",resolution/1000.,resolution/10000.,3*resolution/1000.);//Only for low level fit[8-10]
  RooRealVar sigma("sigma","sigma 4mu mass",resolution/10000.,resolution/10000.,3*resolution/1000.);
  //RooVoigtian Voig("voig","voig PDF",mass,mean,sigma,width);
  RooGaussian gauss("gauss","gaussian PDF", mass, mean, sigma);//for lower level fit
  //------------------------------------------------------------
  // combined hypothesis

  RooRealVar nsig("nsig","nsignal",50,0,100);//initially 50,0,500

  RooRealVar nbkg("nbkg","nbackground",2500,0,4000);//initially 2500,0,4000

  RooAddPdf sum("sum","S+B",RooArgList(gauss,bkg),RooArgList(nsig,nbkg));
...
//-------------------------------
 ...
 //setting the enviroment
  ...
  //nsigz=2.;
  sum.plotOn(Massframe,Name("V"),Components(gauss),LineStyle(kDashed),LineColor(kRed));
